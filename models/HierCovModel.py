...
class AttModel(CaptionModel):
    ...
    def _sample(self, bert_info, bert_feats, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        current_sent_pred = bert_feats[:, 0, :]
        for t in range(self.seq_length + 1):
            ...
            # udpate token prediction            
            new_bert_feats = np.zeros((predicts.shape[0], predicts.shape[1], 768), dtype='float32')
            for b in range(predicts.shape[0]):
                for w in range(predicts.shape[1]):
                    new_bert_feats[b, w, :] = self.loader.word_bert_feats[predicts[b,w]]
            new_bert_feats = new_bert_feats.astype(np.float32)
            current_pred = torch.from_numpy(new_bert_feats).float().squeeze(1).cuda()

            # udpate sentence prediction
            for i, token in enumerate(predicts_bert):               
                total_token_list[i].append(token) 
                if token == '[SEP]':
                    out_records[i].append(t)
                    if len(out_records[i])==1:
                        out_list = [total_token_list[i][n] for n in range(t+1)]
                    else:
                        previous_ind = out_records[i][-2]
                        out_list = [total_token_list[i][n] for n in range(previous_ind+1, t+1)]
                    out_tokens = bc.encode([' '.join(out_list)])
                    current_sent_pred[i] = torch.from_numpy(out_tokens).float().cuda()
